...
| 2026-06-26 | claude | **CYCLE-003 / v0.3.0 NEW SURFACE.** crate shipped 2 more cycles (24 paths; spec info.version lags at 1.2.0, live canonical → 1.3.0). Regenerated types. Added `crate.bandcamp.release({item\|url})→BandcampRelease\|null` (honest-gap present:false→null) + `crate.bandcamp.releases({clusterId})→summary[]` (discriminated union on `object`/`present`); re-exported BandcampRelease/Summary/Response/ArtworkItem/BandcampTrack. resolve `locators` (now 8 platform arrays) + dossier `artwork` flowed in via regen, ZERO code. bandcamp_item_id opaque STRING (never numericize); artwork/track URLs link-only. 88 tests + LIVE keyed check (resolve 8-key locators ✓, releases endpoint ✓). Bumped 0.3.0. Beads C3.1–4 closed. Still pending crate: cycle-079 P1 schema declares (error fields/_links typed/BandcampManifest) → bead C2.3. |
| 2026-06-26 | claude | **CYCLE-002 / v0.2.0 KEY-FIRST.** crate wall LIVE (cycle-078, spec 1.1.0, doc-level ApiKeyAuth; only index+openapi public). Regenerated types @1.1.0 (schemas identical, P1 deferred to cycle-079). Made client key-first (#req requires key by default; only index keyless; beacons bearer-only, no X-API-Key); flipped CRATE_RESOURCES tiers; rewrote contract auth-tier test; surfaced X-RateLimit-* on CrateAPIError; docs+examples key-first; bumped 0.2.0. **82 tests green + LIVE KEYED smoke passed** (staging key, env-only, never committed). Beads C2.1/2/4/5 closed; C2.3 (typed gains) blocked→cycle-079. NEXT: jani publish go (set PUBLISH_ENABLED + NPM_TOKEN, flip private:false). |
| 2026-06-30 | claude | **CYCLE-3 / v1.0.0 — CLUSTER-FIRST v2.** Delivered the whole brief end-to-end against crate `/api/v2` (OpenAPI 2.0.0): regenerated `src/generated` + spec/meta; flipped `API_PREFIX`→`/api/v2`; cluster-first reshape (artist dossier is the hub w/ `discography` pointer-index + `bandcamp_emergence`/`bandcamp_tastemaker`); added `label()` + `?fields=` (default-rich opt-out trim, `400 invalid_fields`); `Deprecation`/`Sunset` warn-once + follow `308` preserving POST; REMOVED master/bandcamp/wayfind/usage/**pagination** + their types; cut `1.0.0` (CHANGELOG/README/AGENTS/llms/docs/explorer all reshaped, single-source generated, drift-guarded). Core `244945c`. **Adversarial review gate (wf_604455ac-3b1, 11 agents)**: 6 confirmed findings fixed — dead `CratePaginationError`+`masters_arity` surface fully excised from errors/tests/docs/explorer; explorer "pagination" feature card replaced. Gate green: typecheck · prettier · 76 tests · build · attw all-🟢 · staleness in-sync · docs:check clean. **OPEN (upstream, not SDK):** crate's `/api/v2` spec `info.description` + `x-concepts` narrative still reference `/api/v1` (59 occ.) and say `info.version 1.4.0` — vendored byte-faithfully (do NOT hand-edit; breaks drift+staleness guards). Report to API team via carrefour#116. **NEXT:** tag `v1.0.0` local (publish HELD — no npm org). |

## Decision Log
<!-- Major decisions with rationale -->
| Date | Decision | Rationale | Source |
|------|----------|-----------|--------|
| 2026-06-30 | Do NOT hand-edit vendored `spec/openapi.json` to scrub upstream's stale `/api/v1` narrative; report upstream instead | The spec is vendored byte-faithfully from crate's live `/api/v2` (drift Tier-1 byte-equality + Tier-2 JCS staleness both depend on it). Editing it breaks both guards and misrepresents what crate serves. The stale narrative is the API team's docs bug, surfaced honestly. | adversarial-review wf_604455ac-3b1 #5/#6 |
| 2026-06-30 | Remove `CratePaginationError`/`isCratePaginationError`/`pagination` kind/`masters_arity` from the public API entirely (no deprecation) for 1.0 | Thrown only by the deleted v1 pagination/masters surface; exporting unreachable errors violates the honest-capability standard. A first stable release ships a clean surface, not deprecation baggage. | adversarial-review #2/#3/#4 |
