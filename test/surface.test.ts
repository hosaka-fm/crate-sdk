...
// validate request shaping + the client's own logic (resolve/artist/bandcamp/guards).
let agent: MockAgent;
...
describe('resolve', () => {
  ...
  it('bare locator "discogs:123" → ?discogs=123', async () => {
    ...
    await new Crate().resolve('discogs:123');
    expect(calls[0].path).toContain('discogs=123');
  });

  it('object { url } → ?url=', async () => {
    mock('GET', (p) => p.startsWith('/api/v1/resolve'), 200, { cluster_id: HEX });
    await new Crate().resolve({ url: 'https://x.bandcamp.com' });
    expect(decodeURIComponent(calls[0].path)).toContain('url=https://x.bandcamp.com');
  });

  it('empty string → CrateValidationError(exactly_one_of), no network', async () => {
    const err = await new Crate().resolve('').catch((e) => e);
  ...
  it('object with two identifiers → CrateValidationError', async () => {
    const err = await new Crate().resolve({ q: 'a', url: 'b' } as never).catch((e) => e);
    expect(isCrateValidationError(err)).toBe(true);
  });
  ...
  it('honest gap (200 + null cluster_id) passes through, does not throw', async () => {
    ...
    const r = await new Crate().resolve({ url: 'https://unknown.example' });
...
describe('artist', () => {
  ...
  it('plain name → one-hop direct /artist/{name}', async () => {
    ...
    await new Crate().artist('Four Tet');
  ...
  it('locator → resolve then /artist/{cluster}', async () => {
    mock('GET', (p) => p.startsWith('/api/v1/resolve'), 200, { cluster_id: HEX });
    mock('GET', (p) => p.startsWith(`/api/v1/artist/${HEX}`), 200, { display: 'Four Tet' });
    await new Crate().artist('discogs:123');
  ...
  it('bare numeric → discogs resolve path (ADX-9)', async () => {
    ...
    await new Crate().artist('1234567');
  ...
  it('locator miss → CrateNotFoundError for artist(); null for artistOrNull()', async () => {
    ...
    const err = await new Crate().artist('discogs:999').catch((e) => e);
...
describe('bandcamp', () => {
  ...
  it('bulkAll paginates rows across pages, terminates on null next_cursor', async () => {
    ...
    const rows = [];
    for await (const row of new Crate().bandcamp.bulkAll({ source: 's' })) rows.push(row);
  ...
  it('maxPages stops cleanly (truncated, no throw) and exposes a resumable cursor', async () => {
    ...
    const handle = new Crate().bandcamp.bulkAll({ source: 's', maxPages: 1 });
    const rows = [];
  ...
  it('non-advancing cursor → CratePaginationError', async () => {
    // every page returns the same non-null cursor (persist so page 2 also replies)
    mock(
      'GET',
      () => true,
      200,
...
describe('search', () => {
  it('serializes array facets as repeat-key + numbers as strings', async () => {
    ...
    await new Crate().search({ genre: ['idm', 'ambient'], year_from: 2000, limit: 5 });
...
describe('auth gating', () => {
  it('key-gated facets() without apiKey → CrateValidationError(api_key_required), no network', async () => {
  ...
  it('sends X-API-Key when constructed with apiKey', async () => {
...
describe('beacon', () => {
  ...
  it('observed() with beaconToken sends Authorization: Bearer and injects timestamp', async () => {
